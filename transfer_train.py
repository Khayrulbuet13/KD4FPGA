...
def main():
    ...
    labels_to_keep = [0, 2]  # Example: Keep two classes
    ...
    # Load pre-trained ResNet50 model and modify it for the task
    resnet50 = models.resnet50()
    num_ftrs = resnet50.fc.in_features
    resnet50.fc = nn.Linear(num_ftrs, len(labels_to_keep))  # Adjust for the number of classes
    resnet50.load_state_dict(torch.load("./checkpoint/Final_models/Teacher_imsize-120_30 September 22:37.pt", map_location=device))
    ...
    for name, param in resnet50.named_parameters():
        if 'fc' not in name:
            ...
            param.requires_grad = True  # Only train the final fully connected layer

    resnet50 = resnet50.to(device)

    # Print and confirm which parameters are being trained
    trainable_params = [param for param in resnet50.parameters() if param.requires_grad]
    print("Trainable parameters:", [name for name, param in resnet50.named_parameters() if param.requires_grad])

    if not trainable_params:
        raise ValueError("No trainable parameters found. Check the model definition and parameter settings.")

    # Define optimizer, model, and callbacks
    optimizer = optim.Adam(trainable_params, lr=params["lr"])
